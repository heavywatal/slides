...
(e-Statの不具合で落とせない場合の予備
<a href="../tohoku2025r/tblT001082C04.txt" download="tblT001082C04.txt">📄ここに置いときます</a>)
</small>
...
🔰 総務省ルールの図「修正後」にもまだ悪い点がたくさん。3つ列挙しよう。
...
1.  Excelで別名のCSVとして保存し、テキストの中身を確認してレポート。
...
- [気象庁](https://www.data.jma.go.jp/gmd/risk/obsdl/index.php)、
  [環境省](https://www.env.go.jp/sogodb/)、
  [農水省](https://www.maff.go.jp/j/tokei/index.html)
...
- 半角スペースが足りない: `##見出し未遂` `-箇条書き未遂` `1.番号付きリスト未遂`
- 日本語の中黒「・」を使った箇条書きもどきはNG。
- 行末 `\` による改行を使った段落もどきはNG。空行で改段落。
- 見た目の装飾はQuartoに任せ、セマンティクス(意味)を意識しよう。
  - `# 文書タイトル` (全体で1つまで。使わないことも)
  - `## 大きなセクションの見出し` (基本はこれだけでも十分)
  - `### その中のサブセクションの見出し`
  - `` `inline code` ``
  - ````
    ```
...
- 様式: Quarto Markdownで書き、Render済みHTMLを提出。
  ...
  - `{ID}-{name}-day{d}.html`, e.g., `C0SB0000-iwasaki-day3.html`
  - `embed-resources: true` で画像などを埋め込んだ独立HTML。
- 評価ポイント
  - 上記様式を守り、エラーも警告も無くコードが走る。
...
- **個人で課題1(組み込みデータの可視化)に取り組む。**
  ...
  - TAが評価して合格した班から解散。
- **最終課題は水曜に発表してもらえればいいので今日は未完成でOK**。
  - 居残って取り組んでもOK。
