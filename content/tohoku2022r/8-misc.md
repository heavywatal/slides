...
サイコロを10回振ったら9回も6の目が出たぞ。イカサマのダイスじゃないか？
...
1. この ***p*-value** が**有意水準(危険率)**$\alpha$より低い場合、帰無仮説を棄却<br>
   (有意水準は大概0.05とか0.01とか)
1. 対立仮説を採択。(6の目の出る確率は1/6と有意に異なる)

???
`10 * 1 / (6**9) * 5 / 6  + 1 / (6**10)`
`choose(20, 9) * 1 / (6**9) * (5 / 6) ** 11  + 1 / (6**10)`

---
## 統計的仮説検定の考え方

サイコロを12回振ったら4回も6の目が出たぞ。<br>
これも1/6よりは高いけど、イカサマのダイスかな？

帰無仮説のもとで12回中<span style="color: #990000;">4回以上6が出る確率: $p = 0.125$</span><br>
(これは二項分布を使って求められる。詳細は省略)
...
データの様子や解析の目的に応じて正しいものを選ぶ必要がある。<br>
が、今回はそこには深入りしない。
