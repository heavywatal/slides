...
penguins_bill_nona = penguins %>% drop_na(bill_length_mm, bill_depth_mm)
p_bill = penguins_bill_nona %>%
...
fit1 = glm(bill_depth_mm ~ bill_length_mm, data = penguins_bill_nona)
fit2 = glm(bill_depth_mm ~ bill_length_mm + species, data = penguins_bill_nona)
fit3 = glm(bill_depth_mm ~ bill_length_mm + species + bill_length_mm:species, data = penguins_bill_nona)
p1 = p_bill +
  geom_line(aes(y = pred), data = modelr::add_predictions(penguins_bill_nona, fit1), size = 1, color = "#3366ff")
p2 = p_bill + aes(color = species) +
  geom_line(aes(y = pred), data = modelr::add_predictions(penguins_bill_nona, fit2), size = 1)
p3 = p_bill + aes(color = species) +
  geom_line(aes(y = pred), data = modelr::add_predictions(penguins_bill_nona, fit3), size = 1)
labels = sprintf("AIC = %.1f", AIC(fit1, fit2, fit3)$AIC)
cowplot::plot_grid(p1 + labs(title = labels[1]),
                   p2 + labs(title = labels[2]) + theme(legend.position = "none"),
                   p3 + labs(title = labels[3]) + theme(legend.position = "none"), nrow = 1L)
