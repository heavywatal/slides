...
**simple linear regression** with a single expl. variable.<br>
**multiple linear regression** with multiple expl. variables.


---
## Multiple regression with multiple explanatory variables
...
- Link function: **identity** (does nothing)
...
---
## Generalized Linear Model (GLM) wrap-up

確率分布・リンク関数を変えて柔軟にモデリングできる。<br>
特定の組み合わせには名前がある。

| Name | Distribution | Link funcrtion | expl. variables |
| ---- | -------- | -------- | -------- |
...
**null model**: 説明変数なし。切片のみ。<br>
**saturated model**: データ点の数 ≤ パラメータの数。“データ読み上げ”的モデル
...
---
## 🔰 Challenge: GLM with fictional data
