...
<span style="color: #888888;">&nbsp; &nbsp; ↓ <span class="fragment highlight-blue custom bold">個体差</span>などの変量効果を扱いたい</span>
...
親1個体あたりの生存数は<span style="color: #0057b7;">n=8の二項分布</span>になるはずだけど、<br>
...
tidy_od |>
  ...
  annotate("text", x = -Inf, y = Inf, hjust = -0.1, vjust = 2, label = label, color = oi$skyblue, size = 6) +
  scale_alpha_identity() +
  scale_fill_manual(values = c(observed = "#333333", expected = oi$skyblue)) +
...
p_z + geom_col(width = 0.1, fill = color_z, alpha = 0.6)
...
p_mixed = df_mixed |>
  ...
  geom_col(data = df_binom, fill = oi$skyblue, alpha = 0.33) +
  facet_wrap(vars(sigma), nrow = 1L, labeller = label_both) +
  labs(x = "# survived seeds") +
  theme_bw(base_size = 20) +
  theme(panel.grid.minor = element_blank(), panel.grid.major.y = element_blank(),
...
p_mixed + geom_col(width = 0.4, fill = color_z, alpha = 0.66)
```
