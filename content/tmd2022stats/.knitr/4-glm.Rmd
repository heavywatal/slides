...
p_pois +
  ...
  ggridges::geom_vridgeline(data = df_bars, aes(x, y, width = density * 0.5, group = x), fill = "#3366ffaa", linetype = 0)
```
...
df_ancova = tibble::tibble(
  ...
  dplyr::select(!cloudy) |>
  dplyr::mutate(beer_sales = rnorm(n, true_coef[1] + true_coef[2] * temperature + true_coef[3] * sunny + true_coef[4] * rainy, 10))
```
...
df_interact = tibble::tibble(
  ...
  dplyr::mutate(name = weather, value = 1L) |>
  tidyr::pivot_wider(values_fill = 0L) |>
  dplyr::mutate(mu = true_coef[1] * sunny + true_coef[2] * temperature + true_coef[3] * rainy + true_coef[4] * temperature * rainy) |>
...
```{r penguins-aic, fig.height = 4, fig.width = 11, warning = FALSE}
...
p1 = p_bill +
  geom_line(aes(y = pred), data = modelr::add_predictions(panguins_bill_nona, fit1, type = "response"), linewidth = 1, color = "#3366ff")
p2 = p_bill + aes(color = species) +
  geom_line(aes(y = pred), data = modelr::add_predictions(panguins_bill_nona, fit2, type = "response"), linewidth = 1)
p3 = p_bill + aes(color = species) +
  geom_line(aes(y = pred), data = modelr::add_predictions(panguins_bill_nona, fit3, type = "response"), linewidth = 1)
labels = sprintf("AIC = %.1f", AIC(fit1, fit2, fit3)$AIC)
cowplot::plot_grid(p1 + labs(title = labels[1]),
                   p2 + labs(title = labels[2]) + theme(legend.position = "none"),
                   p3 + labs(title = labels[3]) + theme(legend.position = "none"), nrow = 1L)
