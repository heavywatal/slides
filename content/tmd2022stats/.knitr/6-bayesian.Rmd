...
tibble::tibble(p = seq(0, 1, 0.01), logLik = dbinom(1, 4, p, log = TRUE)) |>
  ...
  ggplot() + aes(p, logLik) +
  geom_line() +
  geom_point(data = function(.x) {dplyr::slice_max(.x, logLik)}, shape = 16, size = 5, color = oi$skyblue) +
  geom_vline(data = function(.x) {dplyr::slice_max(.x, logLik)}, aes(xintercept = p), color = oi$skyblue) +
...
tibble::tibble(p = seq(0, 1, 0.01), Density = dbeta(p, 2, 4)) |>
  ...
  geom_area(fill = oi$skyblue, alpha = 0.5) +
  theme_bw(base_size = 18) +
  theme(panel.grid.minor = element_blank(), axis.ticks = element_blank())
```
...
tibble::tibble(p = seq(0, 1, 0.02), L = dbinom(1, 4, p)) |>
  ggplot() + aes(p, L) +
  geom_line(color = oi$skyblue, linewidth = 2) +
...
```
  ...
  <div class="column" style="flex-shrink: 1.0;">
<img src="./figure/prior-beta-1.png" alt="plot of chunk prior-beta" width="216">
  </div>
</div>
...
```
  ...
  <div class="column" style="flex-shrink: 1.0">
<img src="./figure/likelihood-binom-1.png" alt="plot of chunk likelihood-binom" width="216">
  </div>
  <div class="column" style="flex-shrink: 4.0; padding-top: 3rem;">
  ⨉
  </div>
...
p_eti = df_density |>
  ...
  geom_area(data = .f, fill = oi$skyblue, alpha = 0.5) +
  ...
  .theme + theme(panel.grid.major = element_line(color = oi$skyblue))
...
p_hdi = df_density |>
  ggplot() + aes(p, Density) +
  geom_hline(yintercept = 0) +
  geom_area(data = df_hdi, fill = oi$skyblue, alpha = 0.5) +
  geom_line(color = "#999999", linewidth = 1) +
...
ggplot(points) + aes(x, y) +
  ...
  geom_point(data = function(.x) dplyr::filter(.x, x ^ 2 + y ^ 2 < 1), color = oi$skyblue, shape = 16, size = 2) +
...
p_pdf = tibble::tibble(p = seq(0, 1, 0.01), Density = (dbeta(p, 4, 6) + dbeta(p, 70, 30)) / 2) |>
  ...
  geom_point(data = df_point |> filter(accepted), color = oi$skyblue, shape = 16, size = 2, alpha = 1) +
  labs(x = "X") +
  theme_bw(base_size = 20) +
  theme(panel.grid.minor = element_blank(), axis.ticks = element_blank(),
...
p_accepted = tibble::tibble(p = runif(n), th = dbeta(p, 4, 6) + dbeta(p, 70, 30)) |>
  ...
  geom_histogram(bins = 20, center = 0.5, fill = oi$skyblue, alpha = 0.5) +
  labs(x = "X") +
  theme_bw(base_size = 20) +
  theme(panel.grid.minor = element_blank(), axis.ticks = element_blank(),
        panel.grid.major.y = element_blank(), panel.border = element_blank(),
...
df_point |>
  ...
  geom_segment(.aes, df_arrow, linejoin = "mitre", linewidth = 1,
    position = position_nudge(0, +0.008), arrow = .ar) +
  geom_segment(.aes, df_arrow, linejoin = "mitre", linewidth = 1,
    position = position_nudge(0, -0.008), arrow = .ar2, alpha = 0.33) +
...
for (i in seq.int(2L, nrow(df_gibbs), 2L)) {
  ...
  p = p_tile +
    geom_path(data = df_i, color = oi$skyblue, alpha = 0.5, size = 1) +
    geom_point(data = df_i, color = oi$skyblue, alpha = 0.5, shape = 16)
  print(p)
}
```
