+++
url = "soken2026biostats/5-content.html"
linktitle = "データ内容の処理: 数値、文字列など。"
title = "5. データ内容の処理: 数値、文字列など。 — 生物統計学 2026 総研大"
...
``` r
df = tibble::tibble(x = c(1, 2, NA), y = c("a", NA, "c"), z = c("D", "E", NA))
df |> dplyr::mutate(y = dplyr::coalesce(y, z))
```
...
``` r
df |> dplyr::mutate(x = dplyr::coalesce(x, y))
```

```
Error in `dplyr::mutate()`:
ℹ In argument: `x = dplyr::coalesce(x, y)`.
...
``` r
x = "This is a string"
y = 'Use single quotes if you want to include a "quote" in a string'
```
...
``` r
stringr::str_pad(fruit4, 8, "left", "_")   # 指定幅にして空白を埋める
```
...
1   0  30   15   29   6  126    3  209     0  JST     NA
```
...
``` r
lubridate::ymd(c("20260729", "2026-07-29", "24/07/29"))
```

```
[1] "2026-07-29" "2026-07-29" "2024-07-29"
```

日時型から単位ごとに値を取得:

``` r
today = lubridate::ymd(20260729)
...
- 誰かに相談しつつ、消化しきれなかった部分をなるべく解消する。
- 構造処理の4つの課題を協力/手分けして解く。
- 課題の出来を評価し、合格した班から個人課題に進む。
- **個人で今日の最終課題(組み込みデータの可視化)に取り組む。**
