...
p1 = ggplot(df_beer) + aes(temperature, beer_sales, color = humidity) +
  geom_line(data = df_aug, aes(y = .fitted, group = humidity), alpha = 0.7) +
...
p2 = ggplot(df_beer) + aes(humidity, beer_sales, color = temperature) +
  geom_line(data = df_aug, aes(y = .fitted, group = temperature), alpha = 0.7) +
  geom_point(alpha = 0.5) +
...
ggplot(df_aug) + aes(x, y_prop) +
  ...
  geom_line(aes(y = .fitted), linewidth = 2, color = "#3366ff") +
  scale_x_continuous(breaks = x_breaks) +
...
broom::augment(glm_bernoulli, type.predict = "response") |>
  ...
  ggridges::geom_vridgeline(aes(x, y, width = density * 6, group = x),
    data = df_bars, fill = "#56B4E9AA", linetype = 0) +
  geom_line(aes(y = .fitted), color = "#3366ff") +
...
p_plant = df_plant |>
  ...
  ggplot() + aes(x, y) +
  geom_line(aes(y = .fitted), linewidth = 1.5, alpha = 0.6) +
  geom_point(shape = 16, alpha = 0.6) +
  theme_bw(base_size = 20) +
  theme(panel.grid.minor = element_blank(), legend.position = "none")
...
#| fig.height: 5
#| fig.width: 5
added1 = broom::augment(fit1, type.predict = "response")
p1 = p_penweight +
  geom_line(aes(y = .fitted), data = added1, linewidth = 1, color = "#3366ff")
p1
```
...
#| fig.height: 5
#| fig.width: 7
added2 = broom::augment(fit2, type.predict = "response")
p2 = p_penweight_color +
  geom_line(aes(y = .fitted), data = added2, linewidth = 1)
p2
```
...
#| fig.height: 5
#| fig.width: 7
added3 = broom::augment(fit3, type.predict = "response")
p3 = p_penweight_color +
  geom_line(aes(y = .fitted), data = added3, linewidth = 1)
p3
```
...
broom::tidy(fit4)
broom::glance(fit4)
added4 = broom::augment(fit4, type.predict = "response")
p_bill + geom_line(aes(y = .fitted, color = sex), data = added4, linewidth = 1) +
