...
if (!fs::file_exists("131105_population_20211001.csv")) {
  dataset = "98331303-9277-4266-bac6-a637ab5e47a5"
  resource = "8019010f-a2b1-4313-bcd8-45ade27d2a3d"
  basename = "131105_population_20211001.csv"
  url = stringr::str_glue("https://data.bodik.jp/dataset/{dataset}/resource/{resource}/download/{basename}")
  raw_meguro = readr::read_csv(url)
  readr::write_csv(raw_meguro, basename, na = "")
}
...
tidy_meguro = raw_meguro |>
  dplyr::select("地域名", matches("の.+性$")) |>
  dplyr::rename(place = 地域名) |>
  dplyr::mutate(place = stringr::str_remove(place, "\\S丁目")) |>
  dplyr::group_by(place) |>
  dplyr::summarize(across(everything(), sum)) |>
  dplyr::ungroup() |>
  tidyr::pivot_longer(!place, names_to = "category", values_to = "count") |>
  tidyr::separate(category, c("age", "sex"), sep = "の") |>
...
#| fig.width: 12
tidy_meguro |>
  dplyr::mutate(count = ifelse(sex == "男性", -1, 1) * count) |>
  ggplot() +
...
sjis = readr::locale(encoding = "SJIS")
...
raw_miyagi = dplyr::bind_cols(miyagi_L, miyagi_R) |> print()
```
...
tidy_miyagi = raw_miyagi |>
  ...
  dplyr::mutate(lower = readr::parse_number(lower), upper = readr::parse_number(upper)) |>
...
<figure>
<img src="../tohoku2022r/image/garbage-in-garbage-out.drawio.svg">
</figure>
...
<figure>
<img src="../tohoku2022r/image/math-model-biased.drawio.svg"><br>
...
<figure>
<img src="../tohoku2022r/image/error-random-systematic.drawio.svg" width="420"><br>
...
<iframe width="400" height="300" src="./7-distribution.html#/32"></iframe>
...
<figure>
<img src="../tohoku2022r/image/hermeneutics-4-1-3.drawio.svg" width="500">
</figure>
...
<figure>
<img src="../tohoku2022r/image/nicholas-cage-drowned.svg" width="100%">
<figcaption><cite>
...
<figure>
<img src="../tohoku2022r/image/hermeneutics-4-1-5.drawio.svg" width="700">
<figcaption><cite>
