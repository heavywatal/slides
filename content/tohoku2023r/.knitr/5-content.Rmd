...
```{r, na-if}
df |>
  dplyr::mutate(x = dplyr::na_if(x, 1), y = dplyr::na_if(y, "a"))
```
...
```r
diamonds |> dplyr::select(matches("^c"))   # starts_with("c")
starwars |> dplyr::select(matches("s$"))   # ends_with("s")
world_bank_pop |>
...
日時型への変換:
```{r, lubridate-parse}
lubridate::ymd(c("20201017", "2020-10-17", "20/10/17"))
```

日時型から単位ごとに値を取得:
```{r, lubridate-get}
today = lubridate::ymd(20201017)
