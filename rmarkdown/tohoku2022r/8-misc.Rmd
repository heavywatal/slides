...
dplyr::bind_rows(
  ...
  scale_fill_identity() +
  annotate("text", x = c(0.5, 1.6, 2.7, 3.8), y = g / 2, label = c("3", "8", "A", "B"), size = 20) +
  coord_fixed() +
  theme_void()
```
...
nsam = 10L
nrep = 20L
y = split(runif(nsam * nrep * 2), ceiling(seq_len(nsam * nrep * 2) / nsam))
tidyr::crossing(repl = seq_len(nrep), x = c("A", "B")) %>%
  dplyr::mutate(y = y) %>%
  tidyr::unnest(y) %>%
  ggplot() + aes(x, y) +
  geom_boxplot() +
