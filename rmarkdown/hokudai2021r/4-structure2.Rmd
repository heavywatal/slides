...
🔰 `economics` データを縦長にして作図しよう

```{r economics, echo = FALSE}
wtl::printdf(economics, n = 4L)
```

```{r economics-longer, echo = FALSE, fig.height = 5, fig.width = 7}
economics %>%
  pivot_longer(!date) %>%
  ggplot() +
  aes(date, value) +
...
<img src="/slides/image/cheatsheet/tidyr-pivot_wider.png" width="600">
<br>
...
🔰 `world_bank_pop` の人口データを使いやすい形にしよう

```{r world_bank_pop}
wtl::printdf(world_bank_pop, n = 4L)
```
```{r world_bank_pop-pivot, echo = FALSE}
world_bank_pop %>%
  ...
  facet_grid(rows = vars(indicator), scale = "free_y")
...
🔰 Rやパッケージに付属のデータで整形→作図しよう
: e.g., `iris`, `diamonds`, `starwars`, `mpg`, `economics`, `txhousing`.
