...
```{cat, engine.opts = list(file = "lm.stan"), class.source = "stan"}
data {
  int<lower=0> N;
  vector<lower=0>[N] x;
  vector[N] y;
...
```{cat, engine.opts = list(file = "lm-credible.stan"), cache.rebuild = !file.exists("lm-credible.stan")}
data {
  int<lower=0> N;
  vector<lower=0>[N] x;
  vector[N] y;
...
```{r stan-lm-generated, include = FALSE, cache.extra = file.mtime("lm-credible.stan")}
n = 300L
a = 3
b = -3
...
model = cmdstan_model("lm-credible.stan")
...
<img height=160 src="figure/multiple-regression-1.png">
<img height=160 src="figure/glm-logistic-1.png">
...
```{cat, engine.opts = list(file = "poisson.stan"), cache.rebuild = !file.exists("poisson.stan")}
data {
  int<lower=0> N;
  vector<lower=0>[N] x;
...
```{cat, engine.opts = list(file = "multiple.stan"), cache.rebuild = !file.exists("multiple.stan")}
data {
  int<lower=0> N;
...
```{cat, engine.opts = list(file = "logistic.stan"), cache.rebuild = !file.exists("logistic.stan")}
data {
  int<lower=0> N;
...
```{cat, engine.opts = list(file = "anova.stan"), cache.rebuild = !file.exists("anova.stan")}
data {
  int<lower=0> N;
...
```{cat, engine.opts = list(file = "ancova.stan"), cache.rebuild = !file.exists("ancova.stan")}
data {
  int<lower=0> N;
  vector<lower=0,upper=1>[N] sunny;
  vector<lower=0,upper=1>[N] rainy;
...
```{cat, engine.opts = list(file = "penguins-lm.stan"), cache.rebuild = !file.exists("penguins-lm.stan")}
data {
  int<lower=0> N;
...
```{cat, engine.opts = list(file = "penguins-multiple.stan"), cache.rebuild = !file.exists("penguins-multiple.stan")}
data {
  int<lower=0> N;
  vector<lower=0>[N] body_mass_g;
  vector<lower=0>[N] flipper_length_mm;
