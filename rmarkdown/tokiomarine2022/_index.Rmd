...
```{r schedule, echo = FALSE, fig.width = 6.5, fig.height = 6.5}
library(ggplot2)
...
ggplot(df) +
  aes(day, start) +
  geom_segment(aes(xend = day, yend = end), size = 3, alpha = 0.6) +
  geom_text(aes(label = label), hjust = 0, nudge_x = 0.05, nudge_y = 500,
            family = "Hiragino Sans W3", size = 5) +
  scale_x_continuous(breaks = c(1, 2)) +
  scale_y_datetime(breaks = .breaks, date_labels = "%H:%M") +
  coord_cartesian(xlim = c(1, 3), ylim = c(day_end, day_start)) +
