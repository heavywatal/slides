...
- 複数chainsで異なる初期値から実行し、軌跡や分布を可視化
- Gelman-Rubin統計量 $\hat R < 1.05$
- Effective Sample Size (ESS) $N_\text{eff} > 100$ per chain

```{r convergence, echo = FALSE, fig.height = 3, fig.width = 11, message = FALSE, warning = FALSE, results = FALSE}
mydata = list(x = rep(10L, 10L))
mydata$N = length(mydata$x)
model_yes = cmdstan_model("converge-yes.stan")
model_no = cmdstan_model("converge-no.stan")
fit_yes = model_yes$sample(data = mydata, show_messages = FALSE, refresh = 0)
fit_no = model_no$sample(data = mydata, show_messages = FALSE, refresh = 0)
s = fit_yes$summary(variables = "lambda")
template = "Rhat = %.4f, n_eff = %.1f"
label_yes = sprintf(template, s[["rhat"]], s[["ess_bulk"]])
s = fit_no$summary(variables = "lambda")
label_no = sprintf(template, s[["rhat"]], s[["ess_bulk"]])
p1 = bayesplot::mcmc_trace(fit_yes$draws("lambda")) + labs(title = label_yes)
p2 = bayesplot::mcmc_trace(fit_no$draws("lambda")) + labs(title = label_no)
...
```{r read-binom-stan, code = 'cat(readr::read_file("coin.stan"))', echo = FALSE}
```
