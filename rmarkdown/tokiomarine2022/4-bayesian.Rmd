...
```{cat, engine.opts = list(file = "converge-yes.stan"), cache.rebuild = !file.exists("converge-yes.stan")}
data {
  int<lower=0> N;
  array[N] int x;
}
...
```
```{cat, engine.opts = list(file = "converge-no.stan"), cache.rebuild = !file.exists("converge-no.stan")}
data {
  int<lower=0> N;
  array[N] int x;
}
...
```
```{r converge-yes, echo = FALSE, dependson = -1, cache.extra = tools::md5sum("converge-yes.stan"), message = FALSE, warning = FALSE, results = FALSE}
p_yes = myplot_trace("converge-yes.stan", mydata)
```
```{r converge-no, echo = FALSE, dependson = -2, cache.extra = tools::md5sum("converge-no.stan"), message = FALSE, warning = FALSE, results = FALSE}
p_no = myplot_trace("converge-no.stan", mydata)
```
...
```{cat engine.opts = list(file = "coin.stan"), class.source = "stan"}
data {
  int<lower=0> N;
  array[N] int x;
}
...
```{r stan-binom-model, cache.extra = tools::md5sum("coin.stan")}
...
```{r stan-binom-sample, results = FALSE}
