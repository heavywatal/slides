...
```{cat, engine.opts = list(file = "converge-yes.stan")}
data {
  int<lower=0> N;
  array[N] int x;
}
...
```
```{cat, engine.opts = list(file = "converge-no.stan")}
data {
  int<lower=0> N;
  array[N] int x;
}
...
```
```{r converge-yes, echo = FALSE, dependson = c(-1, -3), message = FALSE, warning = FALSE, results = FALSE}
p_yes = myplot_trace("converge-yes.stan", mydata)
```
```{r converge-no, echo = FALSE, dependson = c(-2, -3), message = FALSE, warning = FALSE, results = FALSE}
...
```{cat coin-stan, engine.opts = list(file = "coin.stan"), class.source = "stan"}
data {
  int<lower=0> N;
  array[N] int x;
}
...
```{r stan-binom-model-j1, dependson = -1, include = FALSE}
# to suppress "warning: jobserver unavailable: using -j1"
model = cmdstan_model("coin.stan", cpp_options = list("-j" = 1))
```
```{r stan-binom-model, dependson = -1}
