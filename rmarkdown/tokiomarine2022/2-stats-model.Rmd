...
これくらいなら一瞬で計算してもらえる
```{r ols-prepare, echo = FALSE}
height = df_weight[["height"]]
weight = df_weight[["weight"]]
```
```{python ols}
import statsmodels.api as sm
model = sm.OLS(r.weight, sm.add_constant(r.height))
result = model.fit()
result.params
```

```{r weight-lm, echo = FALSE, fig.height = 4.5, fig.width = 4.5}
...
🔰 上のコードを使って `binomial()` や `poisson()` も試してみよう。
...
を参考に、以下のような乱数を作って作図しよう。

- 1%の当たりを狙って100連ガチャを回した場合
- TODO
