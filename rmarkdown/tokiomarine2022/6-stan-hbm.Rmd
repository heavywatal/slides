...
```{cat glmm-stan, engine.opts = list(file = "glmm.stan"), cache.rebuild = !file.exists("glmm.stan"), class.source = "stan"}
data {
  int<lower=0> N;
...
```{r stan-glmm-prep, echo=FALSE, cache.extra = file.mtime("glmm.stan"), message = FALSE, result = FALSE}
ninds = 100L
mu_ind = 0.5
sd_ind = 3
df_od = tibble::tibble(
