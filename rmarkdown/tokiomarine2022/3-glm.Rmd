...
```{python penguins, results = "hide", cache = FALSE}
import statsmodels.api as sm
penguins = sm.datasets.get_rdataset("penguins", "palmerpenguins", True).data
print(penguins)
```

```{r penguins-r, include = FALSE}
...
```{python penguins-print, echo = FALSE}
print(penguins)
```
...
```{r penguins-fit1, include = FALSE}
fit1 = glm(flipper_length_mm ~ body_mass_g, data = penguins)
broom::tidy(fit1)
broom::glance(fit1)
```
```{python penguins-fit1-py, cache = FALSE}
...
p1 = p_bill +
  geom_line(aes(y = pred), data = modelr::add_predictions(panguins_bill_nona, fit1), size = 1, color = "#3366ff")
p2 = p_bill + aes(color = species) +
  geom_line(aes(y = pred), data = modelr::add_predictions(panguins_bill_nona, fit2), size = 1)
p3 = p_bill + aes(color = species) +
  geom_line(aes(y = pred), data = modelr::add_predictions(panguins_bill_nona, fit3), size = 1)
labels = sprintf("AIC = %.1f", AIC(fit1, fit2, fit3)$AIC)
cowplot::plot_grid(p1 + labs(title = labels[1]),
                   p2 + labs(title = labels[2]) + theme(legend.position = "none"),
                   p3 + labs(title = labels[3]) + theme(legend.position = "none"), nrow = 1L)
...
ln = sm.families.links.identity()
fam = sm.families.Gaussian(link=ln)
model = smf.glm(formula, data=penguins, family=fam)
```
...
- TODO: ポアソン回帰の練習に適した課題データ
- TODO: ロジスティック回帰の練習に適した課題データ
